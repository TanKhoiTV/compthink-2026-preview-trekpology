...
function buildMatchmakingOverlay() {
    const el = document.createElement("div");
    el.id = "mm-overlay";
    el.className = "mm-overlay";
    const seats = Array.from({ length: MM_TARGET }, (_, i) => `<span class="mm-seat${i === 0 ? " is-you" : ""}" data-seat="${i}">
...
function setMatchmakingCount(count) {
    const el = document.getElementById("mm-overlay");
    if (!el)
        return;
    const safe = Math.max(1, Math.min(MM_TARGET, count));
    el.querySelectorAll(".mm-seat").forEach((seat, i) => {
        const filled = i < safe;
...
function playMatchmakingTakeoff() {
    ...
    if (title)
        title.innerHTML = `Đủ đội hình — <span>cất cánh!</span>`;
    if (sub)
        sub.textContent = "Đang đưa cả đoàn vào hành trình…";
    if (kicker)
        kicker.innerHTML = `<i class="mm-radar is-on"></i>HOÀN TẤT GHÉP TRẬN`;
    cancel === null || cancel === void 0 ? void 0 : cancel.remove();
...
window.startMatchmaking = function (_btn, city = "SAIGON") {
    var _a;
    if (isSearchingMatch) {
    ...
    const user = authClientState.user;
    const playerName = (user === null || user === void 0 ? void 0 : user.displayName) || (user === null || user === void 0 ? void 0 : user.username) || "Lữ Khách";
    ...
    if (sub)
        sub.textContent = `Đang kết nối những lữ khách cùng chuyến đến ${(_a = CITY_LABELS[city]) !== null && _a !== void 0 ? _a : city}`;
};
...
socket.on("matchmaking:status", (payload) => {
    var _a;
    if (!isSearchingMatch)
        return;
    setMatchmakingCount((_a = payload === null || payload === void 0 ? void 0 : payload.count) !== null && _a !== void 0 ? _a : 1);
});
