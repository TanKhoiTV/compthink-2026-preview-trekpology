import { computeDayCombos } from "./combos.js";
function cardToComboCard(card) {
    var _a, _b;
    const tags = card.tags && card.tags.length > 0
        ? card.tags.map((t) => t.toUpperCase())
        : [String(card.tag).toUpperCase()];
    return { tags, coin: (_a = card.coin) !== null && _a !== void 0 ? _a : 0, stamina: (_b = card.stamina) !== null && _b !== void 0 ? _b : 0 };
}
export function calculateScoreBreakdown({ placedCards, getBoardDisplayName, dayCells, }) {
    ...
    const cells = dayCells !== null && dayCells !== void 0 ? dayCells : placedCards.map((c) => cardToComboCard(c));
...
export function buildSimulationReplaySteps({ boardSlots, currentDayIndex, dayLabel, rows, getCardTagKeys, countCardsWithTag, getCurrentDayPlacedCards, forceTutorialEvent, }) {
    var _a, _b, _c, _d;
    const steps = [];
    ...
    for (let rowIndex = 0; rowIndex < rows.length; rowIndex += 1) {
        ...
        */
        ...
        const distanceEvent = previousCard
            ...
            : null;
        let activeEvent = distanceEvent !== null && distanceEvent !== void 0 ? distanceEvent : randomEvent;
...
export function calculateSimulationResult({ boardSlots, currentDayIndex, dayLabel, rows, getBoardDisplayName, getCardTagKeys, countCardsWithTag, getCurrentDayPlacedCards, forceTutorialEvent, }) {
    // Cột 5 ô của ngày hiện tại (giữ vị trí khung giờ) cho combo theo vị trí.
    const dayCells = boardSlots.map((row) => {
        var _a, _b, _c;
        const card = (_a = row[currentDayIndex]) !== null && _a !== void 0 ? _a : null;
        if (!card)
            return null;
        const tags = card.tags && card.tags.length > 0
            ? card.tags.map((t) => t.toUpperCase())
            : [String(card.tag).toUpperCase()];
        return { tags, coin: (_b = card.coin) !== null && _b !== void 0 ? _b : 0, stamina: (_c = card.stamina) !== null && _c !== void 0 ? _c : 0 };
    });
