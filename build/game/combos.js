...
export function computeDayCombos(dayCells) {
    const placed = dayCells.filter((c) => c != null);
    if (placed.length === 0)
        return { bonus: 0, lines: [] };
    const has = (tag) => placed.filter((c) => c.tags.includes(tag));
    const count = (tag) => has(tag).length;
    let bonus = 0;
    const lines = [];
    const add = (vp, label) => {
        bonus += vp;
    ...
    if (food >= 2)
        add(5, `Combo Ẩm thực x${food}`);
    if (culture >= 2)
        add(8, `Combo Văn hóa x${culture}`);
    if (action >= 2)
        add(10, `Chuỗi Khám phá x${action}`);
    // ── Secondary tags (Trong nhà / Ngoài trời) ───────────
    const indoor = count("INDOOR");
    const outdoor = count("OUTDOOR");
    if (indoor >= 2)
        add(5, `Trong Nhà x${indoor}`);
    if (outdoor >= 2)
        add(6, `Ngoài Trời x${outdoor}`);
    if (indoor >= 1 && outdoor >= 1)
        add(4, "Cân Bằng (trong + ngoài)");
    ...
    if (dayCells[0])
        add(3, "Bình Minh (có lịch sáng)");
    if (dayCells[4])
        add(5, "Cú Đêm (có lịch khuya)");
    if (placed.length >= 4)
        add(7, "Tận Dụng (4+ khung giờ)");
    // ── Risk / Reward (tài nguyên) ────────────────────────
    const totalCoin = placed.reduce((s, c) => { var _a; return s + ((_a = c.coin) !== null && _a !== void 0 ? _a : 0); }, 0);
    const totalStamina = placed.reduce((s, c) => { var _a; return s + ((_a = c.stamina) !== null && _a !== void 0 ? _a : 0); }, 0);
    if (totalCoin <= 2)
        add(6, "Tiết Kiệm (ít Xu)");
    if (totalStamina <= 1)
        add(5, "Khỏe Khoắn (ít Thể lực)");
    if (totalCoin + totalStamina >= 8)
        add(12, "Liều Mạng (chi đậm)");
    return { bonus, lines };
}
export { ROW_LABELS };
